...
import {
  getGeoIpAllowlist,
  createGeoIpAllowlistEntry,
  deleteGeoIpAllowlistEntry,
  getStores
} from '../api/admin'
...
export default function GeoIpAllowlist() {
  const { user } = useAuth()
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      await createGeoIpAllowlistEntry(body)
      toast.success(
        isAllStoresEntry(formStoreCode)
          ? 'IP added to allowlist for all stores'
          : 'IP added to allowlist'
      )
      setIpAddress('')
      setNote('')
      setPage(1)
  ...
  async function handleDelete(entry) {
    ...
    const ok = await confirm({
      ...
      message: allStores
        ? `Remove ${entry.ipAddress} from the allowlist for all stores? Traffic from this IP will be subject to geo-blocking again on every store.`
        : `Remove ${entry.ipAddress} from the allowlist for store "${entry.storeCode}"? Traffic from this IP will be subject to geo-blocking again on that store.`,
      confirmLabel: 'Remove',
      variant: 'danger'
    })
    if (!ok) return
    try {
      await deleteGeoIpAllowlistEntry(entry.id)
  ...
  return (
    <div className="ccw-page">
      <header className="ccw-header">
        <h1 className="ccw-title">Geo IP allowlist</h1>
        <p className="ccw-subtitle">
          IPs on this list bypass geo and VPN blocking for a store&apos;s user site.
          Geo blocking is enforced for every partner store. Choose a single store, or All stores
          to apply the IP everywhere (including stores added later). An allowlisted IP for one
          store does not unlock other stores. Who added each entry is recorded automatically.
        </p>
      </header>
      ...
      <form className="geo-allowlist-form" onSubmit={handleAdd}>
        {isMaster ? (
          <div className="geo-allowlist-form-row">
            <label className="ccw-filter-label" htmlFor="geo-ip-store">
              Store
            </label>
            <select
              id="geo-ip-store"
        ...
        <div className="geo-allowlist-form-row">
          <label className="ccw-filter-label" htmlFor="geo-ip-input">
            IP address / CIDR
          </label>
          <input
            id="geo-ip-input"
        ...
        <div className="geo-allowlist-form-row">
          <label className="ccw-filter-label" htmlFor="geo-ip-note">
            Note (optional)
          </label>
          <input
            id="geo-ip-note"
            ...
            placeholder="e.g. Office Wi‑Fi, Dev VPN"
            maxLength={255}
            disabled={saving}
          />
        </div>
      ...
      <div className="ccw-toolbar" style={{ flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
        {isMaster ? (
          <>
            <label className="ccw-filter-label" htmlFor="geo-ip-filter-store">
              Filter store
            </label>
            <select
              id="geo-ip-filter-store"
              className="ccw-filter-input"
        ...
        <label className="ccw-filter-label" htmlFor="geo-ip-search">
          Search
        </label>
        <input
          id="geo-ip-search"
      ...
      {loading ? (
        ...
        <>
          ...
          {entries.length === 0 && (
            <p className="ccw-subtitle">No allowlisted IPs yet. Add one above to bypass geo-blocking for a store, or for all stores.</p>
          )}
