...
export default function LandingPaymentLinks() {
  const { user } = useAuth()
  ...
  return (
    <div className="store-features-page landing-payment-links-page">
      <h2>Landing payment links</h2>
      <p className="store-features-intro">
        {isMasterAdmin
          ? 'Select a store to view and manage its landing-page deposit and withdrawal dropdown links.'
          : 'Deposit and withdrawal dropdown buttons below the header on your public landing page. Add a label and URL for each option.'}
      </p>

      {isMasterAdmin && (
        <section className="spin-wheel-card" style={{ marginBottom: '1.25rem', padding: '1rem' }}>
          <label htmlFor="landing-payment-links-store" className="store-features-hint" style={{ display: 'block', marginBottom: '0.4rem' }}>
            Select store
          </label>
          <select
            id="landing-payment-links-store"
      ...
      {isMasterAdmin && !storeIdForEditor ? (
        <p className="store-features-hint">
          {storesLoading ? 'Loading stores…' : 'Select a store to manage its landing payment links.'}
        </p>
      ) : (
        <LandingPaymentLinksEditor
          ...
          title={
            isMasterAdmin && selectedStore
              ? `Deposit & withdrawal — ${selectedStore.storeCode || selectedStore.username || selectedStore.userId}`
              : 'Landing page deposit & withdrawal'
          }
          description={
            isMasterAdmin
              ? 'Super admin and technical staff can add, update, or clear deposit and withdrawal links for any store.'
              : "These options appear in the deposit and withdrawal dropdowns on your store's public landing page."
          }
        />
      )}
    </div>
