...
const fingerprintSignupIpAllowlist = require('../../services/auth/fingerprintSignupIpAllowlist.service');

function hasAccess(req) {
  if (req.role === ROLES.DISTRIBUTOR_ADMIN) return false;
  if (req.role === ROLES.MASTER_ADMIN) {
    return canAdmin(req, ADMIN_FEATURE_KEYS.FINGERPRINT_SIGNUP_IP_ALLOWLIST);
  }
  if (req.role === ROLES.STORE_ADMIN) {
    return can(req, STORE_FEATURE_KEYS.FINGERPRINT_SIGNUP_IP_ALLOWLIST);
  }
  return false;
}
...
async function list(req, res) {
  try {
    if (!hasAccess(req)) {
      return sendError(res, 'You do not have access to the fingerprint signup IP allowlist.', 403);
    }
    const data = await fingerprintSignupIpAllowlist.listEntries(req, req.query || {});
...
async function create(req, res) {
  try {
    ...
    const data = await fingerprintSignupIpAllowlist.createEntry(req, req.body || {});
...
async function remove(req, res) {
  try {
    ...
    const data = await fingerprintSignupIpAllowlist.deleteEntry(req, id);
    return sendSuccess(res, data);
  } catch (err) {
