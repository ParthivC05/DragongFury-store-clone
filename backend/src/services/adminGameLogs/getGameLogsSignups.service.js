...
async function getGameLogsSignups({
  scope = {},
  startDate = null,
  endDate = null,
  page = 1,
  ...
  const userAttrs = omitPlayerEmailAttr(
    ['userId', 'username', 'firstName', 'lastName', 'phone', 'email', 'storeCode', 'distributorCode'],
    viewerRole
  );
  const includeUser = Object.keys(userWhere).length > 0
    ? [{ model: db.User, as: 'User', attributes: userAttrs, where: userWhere, required: true }]
  ...
  const { rows, count: total } = await db.GameActivity.findAndCountAll({
    ...
    limit: limitNum,
    offset,
    attributes: ['id', 'userId', 'gameId', 'activityType', 'operationDoneBy', 'operationDoneByUserId', 'createdAt']
  });
  ...
  const list = rows.map((r) => {
    ...
    return {
      ...
      lastName: u.lastName,
      phone: u.phone,
      email: canViewPlayerEmail(viewerRole) ? u.email : undefined,
...
module.exports = { getGameLogsSignups };
