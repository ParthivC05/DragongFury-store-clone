...
const { isMilkywayBotAutomationGame } = require('../../utils/gameIntegration.helpers');

/**
 * Verify the user's current Milkyway password via queryInfo, persist it, and return balance.
 */
async function syncMilkywayGamePassword(userId, gameId, gamePassword, storeCode = null) {
  ...
  if (plainPassword.length < MILKYWAY_PASSWORD_MIN) {
    const err = new Error(`Password must be at least ${MILKYWAY_PASSWORD_MIN} characters.`);
    err.statusCode = 400;
    throw err;
  }
  ...
  if (!isMilkywayAgent) {
    const err = new Error('Password sync is only supported for Milkyway agent games.');
    err.statusCode = 400;
    throw err;
  }
...
module.exports = { syncMilkywayGamePassword };
