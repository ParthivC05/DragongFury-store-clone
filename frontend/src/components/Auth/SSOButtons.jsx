...
/**
 * Renders "Continue with Google" and "Continue with Facebook" when configured.
 * onGoogleSuccess(idToken), onFacebookSuccess(accessToken) are called with the provider token;
 * parent should then call authApi.loginWithGoogle(idToken) / loginWithFacebook(accessToken) and setUserAndToken.
 */
export function SSOButtons({ onGoogleSuccess, onFacebookSuccess, disabled, className = '', googleAuthOptions = {} }) {
  ...
  const renderGoogleButton = useCallback((width) => {
  ...
  // Re-render Google button when container width changes (e.g. mobile layout)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    initFacebookSDK(FACEBOOK_APP_ID)
      ...
      .catch(() => { /* SDK load failed; button stays disabled */ });
  }, []);
  ...
  const ssoButtonBase =
    'w-full py-2.5 px-3 rounded-lg text-sm font-medium flex items-center justify-center gap-2.5 transition-colors disabled:opacity-50';
  ...
  return (
    <div className={className}>
      <div className="relative my-3">
        ...
        <p className="relative flex justify-center text-xs text-gray-500">
      ...
      <div className="flex flex-col gap-2.5">
        {hasGoogle ? (
          useRedirect ? (
            <button
              ...
              className={`relative w-full min-h-[48px] rounded-lg overflow-hidden select-none touch-manipulation flex items-center justify-center gap-2.5 py-2.5 px-3 bg-white border border-gray-200 shadow-sm text-black text-sm font-medium disabled:opacity-60 disabled:pointer-events-none`}
          ...
          <div
            ref={containerRef}
            className={`relative w-full min-h-[48px] rounded-lg overflow-hidden select-none touch-manipulation${ssoDisabled ? ' opacity-60 pointer-events-none' : ''}`}
            aria-busy={ssoDisabled || undefined}
            role="group"
            aria-label="Sign in with Google"
          >
            <div
              className="absolute inset-0 flex items-center justify-center gap-2.5 py-2.5 px-3 rounded-lg bg-white border border-gray-200 shadow-sm text-black text-sm font-medium pointer-events-none"
              aria-hidden
            >
              <svg className="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" aria-hidden>
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            ...
            {/* Invisible official Google button on top so clicks trigger sign-in; opacity-[0.01] so mobile receives touch events */}
            <div
              className="absolute inset-0 z-10 min-w-0 min-h-[48px] opacity-[0.01] [&>div]:!w-full [&>div]:!min-w-0 [&_iframe]:!w-full [&_iframe]:!min-h-[48px] pointer-events-auto cursor-pointer"
          ...
          <button
            type="button"
            disabled
            className={`${ssoButtonBase} border border-gray-600 text-gray-500 cursor-not-allowed`}
        ...
        {/* {hasFacebook ? (
          <button
            ...
            className={`${ssoButtonBase} bg-[#1877F2] text-white hover:bg-[#166FE5] border border-[#1877F2]`}
          ...
          <button
            ...
            aria-label="Sign in with Facebook (unavailable)"
          >
            <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24" aria-hidden>
              <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
            </svg>
